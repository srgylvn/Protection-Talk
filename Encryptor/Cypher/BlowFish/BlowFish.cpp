...
unsigned int CBlowFish::GetOutputArraySize(unsigned int input_array_size)
{
	double res = input_array_size/8.0 - input_array_size/8;
	if (res == 0)
		return input_array_size;
	int full_part = input_array_size/8 + 1;
	double left = full_part - (double)input_array_size/8.0;
	return (unsigned int)(input_array_size + 8.0*left);
}

void CBlowFish::EncryptBuffer(unsigned char* pIn, unsigned int in_size, unsigned char** ppOut, unsigned int &out_size)
{
	int buf_size = GetOutputArraySize(in_size);
	unsigned char * in_buffer;
	
	in_buffer = new unsigned char[buf_size];
	ZeroMemory(in_buffer,buf_size);
	memcpy(in_buffer, pIn, in_size);

	*ppOut = new unsigned char[buf_size+4];

	Encrypt(in_buffer, *ppOut+4, buf_size, CFB);
	out_size = buf_size+4;

	delete in_buffer;
	memcpy(*ppOut, &in_size, 4);	
}

void CBlowFish::DecryptBuffer(unsigned char* pIn, unsigned int iSize, unsigned char** ppOut, unsigned int & iOutSize)
{
	Decrypt(pIn+4, iSize-4, CFB);
	memcpy(&iOutSize, pIn, 4);
	*ppOut = new unsigned char[iOutSize];
	memcpy(*ppOut, pIn+4, iOutSize);
}
